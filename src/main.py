...
class OpenAIAnalyzer:
    """Integrates with OpenRouter + Deepseek API for advanced image analysis"""
    
    def __init__(self, api_key: Optional[str] = None, base_url: Optional[str] = None, model: Optional[str] = None):
        self.api_key = api_key or os.getenv('OPENAI_API_KEY')
        self.base_url = base_url or os.getenv('OPENAI_BASE_URL', 'https://openrouter.ai/api/v1')
        self.model = model or os.getenv('OPENAI_MODEL', 'deepseek/deepseek-r1-0528:free')
        if not self.api_key:
    ...
    def analyze_image_with_gpt4v(self, image_path: str, context: str = "") -> Dict:
        """Analyze image using Deepseek via OpenRouter"""
        if not self.api_key:
        ...
        try:
            ...
            # For text-only models like Deepseek, analyze image features textually
            prompt = f"""
            Analyze this satellite image data for archaeological features:
            Image path: {image_path}
            Context: {context}
            
            Look for:
            1. Geometric patterns indicating human settlement
            ...
            response = client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are an expert archaeologist analyzing satellite imagery for potential archaeological sites."},
            ...
            try:
                ...
                return {
                    ...
                    "confidence": 0.7,
                    "features": ["Analysis provided by Deepseek model"]
                }
...
class ArchaeologicalDetector:
    ...
    def __init__(self, openai_api_key: Optional[str] = None, base_url: Optional[str] = None, model: Optional[str] = None):
        ...
        self.openai_analyzer = OpenAIAnalyzer(openai_api_key, base_url, model)
    ...
    def export_results(self, sites: List[ArchaeologicalSite], base_output_dir: str = 'results'):
        """Export detection results to timestamped files"""
        results = []
        for site in sites:
            results.append({
        ...
        json_file = os.path.join(base_output_dir, f'archaeological_sites_{timestamp}.json')
        csv_file = os.path.join(base_output_dir, f'archaeological_sites_{timestamp}.csv')
...
def main():
    ...
    # Initialize detector with OpenRouter + Deepseek configuration
    api_key = os.getenv('OPENAI_API_KEY')
    base_url = os.getenv('OPENAI_BASE_URL', 'https://openrouter.ai/api/v1')
    model = os.getenv('OPENAI_MODEL', 'deepseek/deepseek-r1-0528:free')
    
    detector = ArchaeologicalDetector(api_key, base_url, model)
    ...
    if os.path.exists(image_dir) and os.listdir(image_dir):
        ...
        json_file, csv_file = detector.export_results(sites)
