...
def visualize_results(df_analysis, df_final):
    ...
    if not df_analysis.empty:
        ...
        plt.tight_layout()
        plt.savefig(CONFIG['RESULTS_DIR'] / 'cv_filtering_results.png', dpi=150, bbox_inches='tight')
        plt.show()
    ...
    if not df_final.empty:
        ...
        if not positive_results.empty:
            ...
            plt.tight_layout()
            plt.savefig(CONFIG['RESULTS_DIR'] / 'llm_discoveries.png', dpi=150, bbox_inches='tight')
            plt.show()
        else:
